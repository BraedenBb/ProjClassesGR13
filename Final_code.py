...
class Patients:
    ...
    def readPatientsFile():
        ...
        for x in range(len(patients)):
            patients_list.append(patients[x].split("_"))
        return patients_list
        
    def editPatientsInfo(doc_list,old_info):
        templist=doc_list
        new_list=[item for item in templist if any(x not in (old_info) for x in item)]
        templist2=new_list
        return templist2
    ...
    def displayPatientsInfo(t):
        patients_list.pop(0)
        head = ["ID", "Name", "Disease", "Gender", "Age"]
        print (tabulate(patients_list, headers=head, tablefmt="fancy_grid"))
...
class Management:
    ...
    while Selection != 0:
        ...
        if Selection == 2:
            ...
            while facilties_input > 3 or facilties_input < 0:
                print()
        ...
        if Selection == 4:
            ...
            while patients_input > 5 or patients_input < 0:
                patients_list=[]
            if patients_input == 1:
                patients_list=Patients.readPatientsFile()
                Patients.displayPatientsInfo(patients_input)
            if patients_input == 2:
            ...
            if patients_input==4:
                ...
                new_info=Patients.enterPatientInfo()
                new_list=Patients.editPatientsInfo(doc_list,old_info)
                Patients.writeListOfPatientsToFile(new_list)
                Patients.addPatientToFile(new_info)
                pass
            if patients_input==5:
                print("\nReturning to Main Menu...\n=========================================================="); break
